...
public class ProductController {
    ...
    public AllProductsResponse allProducts() {
        ...
        AllProductsResponse response = AllProductsResponse.builder().products(products).build();
        return response;
    }
    ...
    public AddProductResponse addProduct(@RequestBody AddProductRequest req, Principal principal) {
        ...
        Product product = Product.builder().name(req.getName()).description(req.getDescription()).price(req.getPrice()).createdByUserEmail(principal.getName()).build();
        Product responseProduct = productService.addProduct(product);
        AddProductResponse response = AddProductResponse.builder().product(responseProduct).success(responseProduct != null).build();
    ...
    public UpdateProductResponse updateProduct(@RequestBody UpdateProductRequest request) {
        Product product = Product.builder().id(request.getId()).name(request.getName()).price(request.getPrice()).description(request.getDescription()).build();
        Product updateProduct = productService.updateProduct(product);
        UpdateProductResponse response = UpdateProductResponse.builder().product(updateProduct).success(updateProduct != null).build();
        return response;
    }

    @DeleteMapping("/{productId}")
    @PreAuthorize("isAuthenticated() and @ACLService.hasPermitToDelete(#productId)")
    public DeleteResponse deleteProduct(@PathVariable long productId) {
        try {
            boolean success = productService.deleteProduct(productId);
            DeleteResponse response = DeleteResponse.builder().success(success).build();
            return response;
        } catch (Exception ex) {
            ex.printStackTrace();
            return null;
